...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<UserDto>> registerUser(@Valid @RequestBody AddEditUserRequest request) {
        return ResponseEntity.ok(ApiResponse.success(UserMessageKey.REGISTRATION_SUCCESSFUL, userFacade.registerUser(request)));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<UserDto>> updateUser(@PathVariable Long id, @RequestBody AddEditUserRequest updateRequest) {
        return ResponseEntity.ok(ApiResponse.success(UserMessageKey.UPDATE_SUCCESS, userService.updateUser(id, updateRequest)));
    }
    ...
    public ResponseEntity<ApiResponse<AllUserResponse>> getAllUsers() {
        return ResponseEntity.ok(ApiResponse.success(GenericMessageKey.REQUEST_SUCCESS, userService.getAllUsers()));
    }
